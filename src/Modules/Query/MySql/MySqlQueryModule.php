...
class MySqlQueryModule extends QueryModule implements Monitored {
	...
	public function interpret($query, $return_type = null) {
		if ($this->queryCanBeInterpreted($query)) {
	 ...
	 * @return bool
	 */
	protected function queryCanBeInterpreted($query): bool {
		if (is_array($query)) {
			foreach ($query as $item) {
				if (!$this->queryCanBeInterpreted($item)) return false;
			}
			return true;
		}
		return !($query instanceof QueryComponent) && !($query instanceof QueryProxy);
	}
}