...
class DataLayer extends StandardLayer {
	...
	public function initStdSmEntityManager(string $classname): SmEntityDataManager {
		...
		if (is_a(DataSource::class, $classname, true)) {
			$dataManager = $this->initDataSourceDataManager();
		} else if (is_a(Model::class, $classname, true)) {
			$dataManager = $this->initModelDataManager();
		} else if (is_a(EntityPropertySchematic::class, $classname, true)) {
			$dataManager = $this->initEntityPropertyDataManager();
		} else if (is_a(Entity::class, $classname, true)) {
			$dataManager = $this->initEntityDataManager();
		} else if (is_a(Property::class, $classname, true)) {
			$dataManager = $this->initPropertyDataManager();
		}
		...
		if (isset($this->layerRoot)) {
			$logger = $this->layerRoot->getLayers()->logging;
			if (isset($logger)) {
