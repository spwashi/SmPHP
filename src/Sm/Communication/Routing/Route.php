...
class Route extends FunctionResolvable implements \JsonSerializable {
	...
	public function resolve($_ = null) {
		...
		$routeDoesNotMatchException = new RouteDoesNotMatchException("Cannot match route with this request");
		$routeDoesNotMatchException->setAttemptedRequest($this->primedRequest)
		                           ->setRoute($this);
		throw $routeDoesNotMatchException;
	}
	####################################################
	#   Setters/Getters
	####################################################
