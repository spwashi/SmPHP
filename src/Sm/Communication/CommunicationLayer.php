...
class CommunicationLayer extends StandardLayer {
	...
	public function email(...$args) :Email{
		/** @var EmailCommunicationModule $emailModule */
		$emailModule =  $this->getModule(CommunicationLayer::MODULE_EMAIL);
		if (!$emailModule) throw new MissingModuleException("Missing an Email Module");

		return $emailModule->resolveEmailCreator(...$args);
	}
	public function checkCanRegisterModule(Module $module, $name): void {
		...
		if (!in_array($name, $expected_modules)) {
			...
			throw new InvalidModuleException("Cannot register module {$name} on layer {$st_class} (need to register)");
		}
	}
	...
	protected function _listExpectedModules(): array {
		return [CommunicationLayer::MODULE_ROUTING,
		        CommunicationLayer::MODULE_HTTP,
		        CommunicationLayer::MODULE_EMAIL];
	}
